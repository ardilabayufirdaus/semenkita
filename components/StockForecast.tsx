...
const generateDataForPlant = (plantId: string) => {
  ...
  const forecastData = [
    { month: 'Oct W1', actual: isCritical ? 1200 : 2100, forecast: 2050, capacity: baseCapacity },
    { month: 'Oct W2', actual: isCritical ? 1100 : 2350, forecast: 2200, capacity: baseCapacity },
    { month: 'Oct W3', actual: isCritical ? 1300 : 2150, forecast: 2300, capacity: baseCapacity },
    { month: 'Oct W4', actual: isCritical ? 900 : 2480, forecast: 2400, capacity: baseCapacity },
  ...
  const siloData = Array.from({ length: plant.siloCount }).map((_, i) => ({
    ...
    current: isCritical && i === 1 ? 800 : Math.floor(plant.siloCapacity * (0.4 + (Math.random() * 0.5))),
  ...
  // 3. Packaging Data
  const packagingData = [
    { material: '50KG-HDPE', stock: isCritical ? 15000 : 125000, dailyUsage: 5000, reorderPoint: 50000, pendingPO: 100000 },
    { material: '50KG-PP', stock: 45000, dailyUsage: 8000, reorderPoint: 60000, pendingPO: 200000 },
    { material: '40KG-PP', stock: 85000, dailyUsage: 2500, reorderPoint: 20000, pendingPO: 0 },
    { material: '1T-JUMBO', stock: 450, dailyUsage: 50, reorderPoint: 200, pendingPO: 500 },
  ];
...
const StockForecast: React.FC = () => {
  ...
  const [insight, setInsight] = useState<string>('Initializing SemenKita AI Analysis...');
  const [loading, setLoading] = useState<boolean>(true);
  const [period, setPeriod] = useState<string>('Weekly');

  // Derived Data based on selection
  const currentPlant = mockPlants.find(p => p.id === selectedPlant) || mockPlants[0];
  const { forecastData, siloData, packagingData } = useMemo(() => generateDataForPlant(selectedPlant), [selectedPlant]);
  ...
  const totalCementStock = siloData.reduce((acc, curr) => acc + curr.current, 0);
  const criticalBags = packagingData.filter(p => p.stock < p.reorderPoint).length;
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center sticky top-0 z-10">
        <div className="flex items-center space-x-4 w-full md:w-auto">
             ...
             <div>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Packing Plant Location</label>
                <div className="relative">
                    ...
                    >
                        {mockPlants.map(p => (
                            <option key={p.id} value={p.id}>{p.plantCode} - {p.plantName}</option>
                        ))}
                    </select>
        ...
        <div className="mt-4 md:mt-0 flex items-center space-x-2 text-sm text-slate-500">
            ...
            <span className="font-mono font-medium">Planning Horizon: OCT 2023 - NOV 2023</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Total Cement Stock ({currentPlant.plantCode})</p>
            <div className="flex items-baseline mt-2">
                <h3 className="text-3xl font-black text-slate-800">{totalCementStock.toLocaleString()}</h3>
            ...
            <div className="mt-4 flex items-center text-xs font-medium text-cyan-700 bg-cyan-50 w-fit px-2 py-1 rounded border border-cyan-100">
                ...
                Capacity Utilization: {Math.round((totalCementStock / (currentPlant.siloCapacity * currentPlant.siloCount)) * 100)}%
            </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Bag Critical Alert</p>
            <div className="flex items-baseline mt-2">
                <h3 className={`text-3xl font-black ${criticalBags > 0 ? 'text-red-600' : 'text-slate-800'}`}>{criticalBags}</h3>
                <span className="text-sm font-bold text-slate-500 ml-1">Materials</span>
            </div>
             <div className={`mt-4 flex items-center text-xs font-medium w-fit px-2 py-1 rounded border ${criticalBags > 0 ? 'text-red-600 bg-red-50 border-red-100' : 'text-emerald-600 bg-emerald-50 border-emerald-100'}`}>
                {criticalBags > 0 ? <AlertTriangle className="w-3 h-3 mr-1" /> : <Package className="w-3 h-3 mr-1" />}
                {criticalBags > 0 ? 'Below Reorder Point' : 'Stock Levels Healthy'}
            </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden group">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Forecast Accuracy</p>
            <div className="flex items-baseline mt-2">
                <h3 className="text-3xl font-black text-slate-800">94.2%</h3>
                <span className="text-sm font-bold text-slate-500 ml-1">Last Month</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
             ...
             <div className="flex justify-between items-center mb-6">
                <div>
                    <h3 className="text-lg font-bold text-slate-800 flex items-center">
                        ...
                        <div className="ml-2 px-2 py-0.5 rounded-full bg-cyan-50 text-cyan-600 text-[10px] font-bold border border-cyan-100">AI MODEL V2.1</div>
                    </h3>
                    <p className="text-xs text-slate-400 font-mono">Comparing Actual Dispatch vs AI Prediction ({currentPlant.plantName})</p>
                </div>
                    ...
                    {['Weekly', 'Monthly'].map((p) => (
                        <button 
                            ...
                            className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${
                                period === p ? 'bg-white shadow-sm text-cyan-700' : 'text-slate-500 hover:text-slate-700'
                            }`}
                        >
                            {p}
                        </button>
            ...
            <div className="h-[320px]">
                <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={forecastData}>
                        ...
                        <Area type="monotone" dataKey="capacity" name="Plant Capacity" stroke="none" fill="url(#colorCapacity)" />
                        <Bar dataKey="actual" name="Actual Dispatch" barSize={30} fill="url(#colorActual)" radius={[4, 4, 0, 0]} />
                        <Line type="monotone" dataKey="forecast" name="AI Forecast" stroke="#f59e0b" strokeWidth={3} dot={{r: 4}} activeDot={{r: 6}} strokeDasharray="5 5" />
                    </ComposedChart>
                </ResponsiveContainer>
            </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col">
            <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center">
                ...
                Silo Levels ({currentPlant.plantCode})
            </h3>
            ...
            <div className="flex-1 flex justify-center items-end space-x-8 px-4">
                {siloData.map((silo) => {
                    ...
                    const colorClass = isCritical ? 'bg-rose-500' : pct < 40 ? 'bg-amber-400' : 'bg-slate-500';
                    const glowClass = isCritical ? 'shadow-[0_0_15px_rgba(244,63,94,0.4)]' : 'shadow-[0_0_15px_rgba(100,116,139,0.2)]';
                    
                    return (
                        <div key={silo.id} className="flex flex-col items-center group w-24">
                            {/* Hover Details */}
                            <div className="mb-2 opacity-0 group-hover:opacity-100 transition-opacity text-center absolute -mt-16 bg-slate-900 text-cyan-50 text-[10px] p-2 rounded shadow-lg z-10 w-32 -ml-4 border border-slate-700">
                                <p className="font-bold text-cyan-400">{silo.current.toLocaleString()} Ton</p>
                                <p className="text-slate-400">Cap: {silo.capacity.toLocaleString()}</p>
                            </div>

                            {/* The Silo Visual */}
                            <div className={`relative w-full h-48 bg-slate-100 rounded-lg border-2 border-slate-300 overflow-hidden flex flex-col justify-end ${glowClass} transition-shadow duration-500`}>
                                {/* Fill Level */}
                                <div 
                                ...
                                <div className="absolute inset-0 flex flex-col justify-between py-2 pointer-events-none">
                                    {[...Array(5)].map((_, i) => (
                                        <div key={i} className="w-full border-t border-slate-400/30 h-0"></div>
                                    ))}
                                </div>
                                <div className="absolute inset-0 flex items-center justify-center">
                                    <span className="text-lg font-black text-slate-800 mix-blend-overlay drop-shadow-md">{pct}%</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-3 text-center">
                                <p className="text-xs font-bold text-slate-700">{silo.name}</p>
                                <p className={`text-[10px] font-bold ${isCritical ? 'text-red-600 animate-pulse' : 'text-emerald-600'}`}>
                                    {isCritical ? 'ATTENTION' : 'OPTIMAL'}
                                </p>
                            </div>
                        </div>
                    );
                ...
                <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs">
                        <tr>
                            ...
                            <th className="p-3 text-right">Daily Usage</th>
                            <th className="p-3 text-center">Days Cover</th>
                            <th className="p-3">Status</th>
                            <th className="p-3 text-right">Pending PO</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {packagingData.map((item, idx) => {
                            const doc = parseFloat(calculateDaysCover(item.stock, item.dailyUsage));
                            const statusColor = doc < 7 ? 'bg-red-100 text-red-700 border-red-200' : doc < 14 ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200';
                            
                            return (
                                <tr key={idx} className="hover:bg-slate-50">
                                    <td className="p-3 font-bold text-slate-700">{item.material}</td>
                                    <td className="p-3 text-right font-mono">{item.stock.toLocaleString()}</td>
                                    <td className="p-3 text-right font-mono text-slate-500">{item.dailyUsage.toLocaleString()}</td>
                                    <td className="p-3 text-center">
                                        <div className="flex flex-col items-center">
                                            <span className="font-black text-slate-800">{doc} Days</span>
                                            <div className="w-16 h-1.5 bg-slate-200 rounded-full mt-1 overflow-hidden">
                                                <div className={`h-full ${doc < 7 ? 'bg-red-500' : 'bg-emerald-500'}`} style={{width: `${Math.min(doc * 3, 100)}%`}}></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-3">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold border uppercase ${statusColor}`}>
                                            {doc < 7 ? 'Critical' : doc < 14 ? 'Reorder' : 'Safe'}
                                        </span>
                                    </td>
            ...
            <div className="flex-1 bg-slate-800/40 rounded-lg p-4 border border-cyan-900/30 overflow-y-auto max-h-[300px] scrollbar-thin scrollbar-thumb-slate-700 relative z-10 shadow-inner">
                {loading ? (
                    <div className="flex flex-col items-center justify-center h-full space-y-3 opacity-70">
                        ...
                        <span className="text-xs font-mono text-cyan-200">Processing supply chain data...</span>
                    </div>
                ) : (
