...
const initialPackTypes: PackType[] = [
  {
    ...
    name: "50 Kg HDPE Bag",
    description:
      "Kantong High-Density Polyethylene (HDPE) laminasi, kuat, tahan air, biasanya digunakan untuk ekspor atau proyek besar.",
  },
  {
    id: "2",
    code: "50KG-PP",
    ...
    description:
      "Kantong anyaman Polipropilen (PP) laminasi, standar pasar domestik Indonesia, paling umum untuk ritel dan proyek menengah.",
  },
  {
    id: "3",
    code: "50KG-AD*",
    ...
    description:
      "Kantong kertas AD* Starlinger (multiwall paper + lapis PP dalam), sering digunakan untuk semen putih atau produk premium.",
  },
  {
    id: "4",
    code: "40KG-PP",
    ...
    description:
      "Kantong anyaman PP 40 kg, umum di beberapa daerah (Sumatera, Kalimantan) atau untuk produk khusus (semen instan/mortar).",
  },
  {
    id: "5",
    code: "1T-JUMBO",
    ...
    description:
      "Big bag / FIBC 1.000 kg, dilengkapi 4 lifting loop + discharge spout, untuk pengiriman curah ke RMC plant, precast, atau ekspor.",
  },
  {
    id: "6",
    code: "1.5T-JUMBO",
    ...
    description:
      "Big bag kapasitas 1.500 kg, khusus untuk proyek besar atau pengiriman kapal curah yang tetap memerlukan kantong.",
  },
  {
    id: "7",
    code: "BULK",
    name: "Bulk / Curah",
    description:
      "Pengiriman langsung tanpa kantong menggunakan truk tangki semen (bulker) atau kapal curah, paling ekonomis untuk beton readymix.",
  },
  {
    id: "8",
    code: "25KG-PP",
    name: "25 Kg PP Bag",
    description:
      "Kantong 25 kg (khususnya untuk semen instan, mortar, tile adhesive, atau pasar ritel premium).",
  },
  {
    id: "9",
    code: "50KG-BIGBAG",
    ...
    description:
      "Kantong sling 50 kg (bentuk mini jumbo bag), sering digunakan untuk semen putih atau produk khusus agar mudah diangkat forklift.",
  },
  // ... other pack types
];
...
const MasterData: React.FC = () => {
  ...
  // Cement State
  const [cementTypes, setCementTypes] =
    useState<CementType[]>(initialCementTypes);
  ...
  const [bagBrokenTypes, setBagBrokenTypes] = useState<BagBrokenType[]>(
    initialBagBrokenTypes
  );

  // Downtime Type State
  const [downtimeTypes, setDowntimeTypes] =
    useState<DowntimeType[]>(initialDowntimeTypes);
  ...
  const [packTypeFormData, setPackTypeFormData] = useState<
    Omit<PackType, "id">
  >({
    code: "",
    name: "",
  ...
  const [bagBrokenFormData, setBagBrokenFormData] = useState<
    Omit<BagBrokenType, "id">
  >({
    category: "",
    area: "",
  });
  ...
  const handleDeletePlant = (id: string) => {
    if (
      window.confirm(
        t("confirm_delete").replace("{entity}", t("plant_singular"))
      )
    ) {
  ...
  const handleDeleteCement = (id: string) => {
    if (
      window.confirm(
        t("confirm_delete").replace("{entity}", t("cement_type_singular"))
      )
    ) {
  ...
  const handleDeletePackType = (id: string) => {
    if (
      window.confirm(
        t("confirm_delete").replace("{entity}", t("pack_type_singular"))
      )
    ) {
  ...
  const handleDeleteBagType = (id: string) => {
    if (
      window.confirm(
        t("confirm_delete").replace("{entity}", t("bag_type_singular"))
      )
    ) {
  ...
  const handleDeleteBagBroken = (id: string) => {
    if (
      window.confirm(
        t("confirm_delete").replace("{entity}", t("bag_broken_type_singular"))
      )
    ) {
  ...
  const handleDeleteDowntime = (id: string) => {
    if (
      window.confirm(
        t("confirm_delete").replace("{entity}", t("downtime_type_singular"))
      )
    ) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === "PLANTS") {
      if (editingId) {
        setPlants(
          plants.map((p) =>
            p.id === editingId ? { ...plantFormData, id: editingId } : p
          )
        );
      } else {
        const newPlant: PackingPlant = {
          ...plantFormData,
      ...
      if (editingId) {
        setCementTypes(
          cementTypes.map((c) =>
            c.id === editingId ? { ...cementFormData, id: editingId } : c
          )
        );
      } else {
        const newCement: CementType = {
          ...cementFormData,
          id: Math.random().toString(36).substr(2, 9).toUpperCase(),
        };
      ...
      if (editingId) {
        setPackTypes(
          packTypes.map((p) =>
            p.id === editingId ? { ...packTypeFormData, id: editingId } : p
          )
        );
      } else {
      ...
      if (editingId) {
        setBagTypes(
          bagTypes.map((b) =>
            b.id === editingId ? { ...bagTypeFormData, id: editingId } : b
          )
        );
      } else {
      ...
      if (editingId) {
        setBagBrokenTypes(
          bagBrokenTypes.map((b) =>
            b.id === editingId ? { ...bagBrokenFormData, id: editingId } : b
          )
        );
      } else {
        const newBagBroken: BagBrokenType = {
          ...bagBrokenFormData,
          id: Math.random().toString(36).substr(2, 9).toUpperCase(),
        };
      ...
      if (editingId) {
        setDowntimeTypes(
          downtimeTypes.map((d) =>
            d.id === editingId ? { ...downtimeFormData, id: editingId } : d
          )
        );
      } else {
        const newDowntime: DowntimeType = {
          ...downtimeFormData,
          id: Math.random().toString(36).substr(2, 9).toUpperCase(),
        };
  ...
  const getModalTitle = () => {
    const action = editingId ? t("edit") : t("create");
    if (activeTab === "PLANTS") return `${action} ${t("plant_singular")}`;
    if (activeTab === "CEMENT") return `${action} ${t("cement_type_singular")}`;
    if (activeTab === "PACK_TYPE")
      return `${action} ${t("pack_type_singular")}`;
    if (activeTab === "BAG_TYPE") return `${action} ${t("bag_type_singular")}`;
    if (activeTab === "BAG_BROKEN")
      return `${action} ${t("bag_broken_type_singular")}`;
    return `${action} ${t("downtime_type_singular")}`;
  };
  ...
  const getTabClass = (tabName: TabView) => {
    const baseClass =
      "px-3 py-2 text-xs font-bold uppercase tracking-wide rounded-sm transition-all flex items-center whitespace-nowrap";
  ...
  const getTabLabel = (tabName: TabView) => {
    if (tabName === "PLANTS") return t("plants");
    if (tabName === "CEMENT") return t("cement");
    if (tabName === "PACK_TYPE") return t("packs");
    if (tabName === "BAG_TYPE") return t("bags");
    if (tabName === "BAG_BROKEN") return t("broken");
    return t("downtime");
  };
    ...
    <div className="space-y-6 h-full flex flex-col">
        ...
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 flex items-center uppercase tracking-tight">
              ...
              {t("master_data_management")}
            </h2>
            <p className="text-sm text-slate-600 mt-1 font-mono">
              {t("mode_label")}: {getTabLabel(activeTab)} • {t("records_label")}
              :{" "}
              {activeTab === "PLANTS"
                ? plants.length
                : activeTab === "CEMENT"
          ...
          <div className="flex bg-slate-200 p-1 rounded-md overflow-x-auto max-w-2xl">
            <button
              ...
              className={getTabClass("PLANTS")}
            >
              <Factory className="w-3 h-3 mr-2" /> {getTabLabel("PLANTS")}
            </button>
            <button
              onClick={() => setActiveTab("CEMENT")}
              className={getTabClass("CEMENT")}
            >
              <Layers className="w-3 h-3 mr-2" /> {getTabLabel("CEMENT")}
            </button>
            <button
              onClick={() => setActiveTab("PACK_TYPE")}
              className={getTabClass("PACK_TYPE")}
            >
              <Package className="w-3 h-3 mr-2" /> {getTabLabel("PACK_TYPE")}
            </button>
            <button
              onClick={() => setActiveTab("BAG_TYPE")}
              className={getTabClass("BAG_TYPE")}
            >
              <ShoppingBag className="w-3 h-3 mr-2" /> {getTabLabel("BAG_TYPE")}
            </button>
            <button
              onClick={() => setActiveTab("BAG_BROKEN")}
              className={getTabClass("BAG_BROKEN")}
            >
              <FileWarning className="w-3 h-3 mr-2" />{" "}
              {getTabLabel("BAG_BROKEN")}
            </button>
            <button
              onClick={() => setActiveTab("DOWNTIME")}
              className={getTabClass("DOWNTIME")}
            >
              <Timer className="w-3 h-3 mr-2" /> {getTabLabel("DOWNTIME")}
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded border border-slate-300 flex-1 flex flex-col shadow-sm overflow-hidden">
        <div className="overflow-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-100 text-slate-700 font-bold text-xs uppercase tracking-wider sticky top-0 z-10 shadow-sm border-b border-slate-300">
              {activeTab === "PLANTS" ? (
                <tr>
                  <th className="p-4 w-1/6 border-r border-slate-200">
                    {t("plant_name_label")}
                  </th>
                  <th className="p-4 w-1/12 border-r border-slate-200">
                    {t("plant_code_label")}
                  </th>
                  <th className="p-4 w-1/6 border-r border-slate-200">
                    {t("capacity_ton_hr")}{" "}
                ...
                <tr>
                  ...
                  <th className="p-4 w-1/4 border-r border-slate-200">
                    {t("cement_name")}
                  </th>
                  <th className="p-4 border-r border-slate-200">
                    {t("description_label")}
                  </th>
                  <th className="p-4 text-right w-24">{t("actions")}</th>
                </tr>
              ) : activeTab === "PACK_TYPE" ? (
                <tr>
                  <th className="p-4 w-1/6 border-r border-slate-200">
                ...
                <tr>
                  <th className="p-4 w-1/5 border-r border-slate-200">
                    {t("material_no")}
                  </th>
                  <th className="p-4 w-2/5 border-r border-slate-200">
                    {t("description_label")}
                  </th>
                  <th className="p-4 border-r border-slate-200 text-right">
                    {t("price_rp")}
                  </th>
                  <th className="p-4 text-right w-24">{t("actions")}</th>
                </tr>
                  ...
                  plants.map((plant) => (
                    ...
                    >
                      ...
                      <td className="p-4 border-r border-slate-100 group-hover:border-slate-200">
                        <span className="font-mono font-bold text-cyan-700 bg-cyan-50 px-2 py-1 rounded text-xs border border-cyan-100">
                          {plant.plantCode}
                        </span>
                      </td>
                      <td className="p-4 border-r border-slate-100 group-hover:border-slate-200">
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          ...
                          className="text-slate-500 hover:text-blue-700 p-1.5 rounded"
                        >
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleDeletePlant(plant.id)}
                          className="text-slate-500 hover:text-red-700 p-1.5 rounded"
                        >
              ...
              {activeTab === "CEMENT" &&
                (cementTypes.length === 0 ? (
                  ...
                  cementTypes.map((cement) => (
                    ...
                    >
                      <td className="p-4 border-r border-slate-100 group-hover:border-slate-200">
                        <span className="font-mono font-bold text-violet-800 bg-violet-50 px-2 py-1 rounded text-xs border border-violet-100">
                          {cement.code}
                        </span>
                      </td>
                      <td className="p-4 font-bold text-slate-800 border-r border-slate-100 group-hover:border-slate-200">
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          onClick={() => handleOpenCementModal(cement)}
                          className="text-slate-500 hover:text-blue-700 p-1.5 rounded"
                        >
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleDeleteCement(cement.id)}
                          className="text-slate-500 hover:text-red-700 p-1.5 rounded"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
              ...
              {activeTab === "PACK_TYPE" &&
                (packTypes.length === 0 ? (
                  ...
                  packTypes.map((pack) => (
                    ...
                    >
                      <td className="p-4 border-r border-slate-100 group-hover:border-slate-200">
                        <span className="font-mono font-bold text-amber-800 bg-amber-50 px-2 py-1 rounded text-xs border border-amber-100">
                          {pack.code}
                        </span>
                      </td>
                      <td className="p-4 font-bold text-slate-800 border-r border-slate-100 group-hover:border-slate-200">
                      ...
                      <td
                        className="p-4 text-slate-600 border-r border-slate-100 group-hover:border-slate-200 truncate max-w-xs"
                      ...
                      <td className="p-4 text-right space-x-2">
                        <button
                          onClick={() => handleOpenPackTypeModal(pack)}
                          className="text-slate-500 hover:text-blue-700 p-1.5 rounded"
                        >
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleDeletePackType(pack.id)}
                          className="text-slate-500 hover:text-red-700 p-1.5 rounded"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
        ...
        <div className="bg-slate-50 p-3 border-t border-slate-300 text-xs text-slate-600 flex justify-between font-mono">
          <span>
            {t("table_label")}: {activeTab}_MASTER
          </span>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              {activeTab === "PLANTS" ? (
                ...
                <div className="text-center py-4 text-slate-500 italic">
                  {t("form_loaded_for")} {getTabLabel(activeTab)}
                </div>
              )}
