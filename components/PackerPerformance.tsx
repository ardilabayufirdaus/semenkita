...
const mockPackerData: PackerMetric[] = [
  {
    id: "1",
    unitName: "Packer 01 (8-Spout)",
    operator: "Shift A - Team",
    productType: "PCC 50kg",
    totalBags: 12500,
    tonnage: 625,
    burstRate: 0.15,
    weightAccuracy: 50.12,
    status: "Running",
    efficiency: 94,
  },
  {
    id: "2",
    unitName: "Packer 02 (8-Spout)",
    operator: "Shift A - Team",
    productType: "OPC 50kg",
    totalBags: 11000,
    tonnage: 550,
    burstRate: 0.85,
    weightAccuracy: 50.35,
    status: "Running",
    efficiency: 88,
  },
  {
    id: "3",
    unitName: "Packer 03 (Jumbo)",
    operator: "Shift A - Team",
    productType: "PCC 1 Ton",
    totalBags: 450,
    tonnage: 450,
    burstRate: 0.05,
    weightAccuracy: 1002.5,
    status: "Maintenance",
    efficiency: 65,
  },
  {
    id: "4",
    unitName: "Packer 04 (8-Spout)",
    operator: "Shift B - Team",
    productType: "PCC 40kg",
    totalBags: 14000,
    tonnage: 560,
    burstRate: 0.22,
    weightAccuracy: 40.05,
    status: "Running",
    efficiency: 97,
  },
];
...
const PackerPerformance: React.FC = () => {
  const { t } = useLanguage();
  const [analysis, setAnalysis] = useState<string>(t("analyzing_telemetry"));
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-emerald-300 transition-colors">
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-800 mt-1">
              91.5 <span className="text-sm font-normal text-slate-500">%</span>
            </h3>
          </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-rose-300 transition-colors">
          <div>
            ...
            <h3 className="text-2xl font-black text-rose-600 mt-1">
              0.31 <span className="text-sm font-normal text-slate-500">%</span>
            </h3>
          </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-6 relative overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-bold text-slate-800 flex items-center">
              ...
              {t("production_vs_burst")}
            </h3>
            <div className="flex space-x-2">
              <span className="flex items-center text-xs text-slate-500">
                <div className="w-2 h-2 bg-cyan-500 mr-1"></div>{" "}
                {t("output_ton")}
              </span>
              <span className="flex items-center text-xs text-slate-500">
                <div className="w-2 h-2 bg-rose-500 mr-1"></div>{" "}
                {t("burst_pct")}
              </span>
            </div>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-white border-b border-slate-100 text-slate-500 font-semibold uppercase tracking-wider text-xs">
              <tr>
                <th className="p-4">{t("machine_unit")}</th>
                <th className="p-4">{t("product")}</th>
                <th className="p-4 text-right">{t("output_bags")}</th>
                <th className="p-4 text-right">{t("weight_avg")}</th>
                <th className="p-4 text-right">{t("efficiency_label")}</th>
                ...
                >
                  ...
                  <td className="p-4 text-right font-mono">
                    <span
                      className={`${
                        Math.abs(
                          packer.weightAccuracy -
                            (packer.productType.includes("40")
                              ? 40
                              : packer.productType.includes("1 Ton")
                              ? 1000
                              : 50)
                        ) > 0.2
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end">
                      <span className="text-xs font-bold mr-2 text-slate-600">
                        {packer.efficiency}%
                      </span>
                      <div className="w-16 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${
                            packer.efficiency > 90
                              ? "bg-cyan-500"
                              : "bg-amber-500"
                          }`}
                          style={{ width: `${packer.efficiency}%` }}
                        ></div>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 text-center">
                    <span
                      className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${
                        packer.status === "Running"
                          ? "bg-emerald-50 text-emerald-700 border-emerald-100"
                          : packer.status === "Maintenance"
                          ? "bg-rose-50 text-rose-700 border-rose-100"
                          : "bg-slate-100 text-slate-600 border-slate-200"
                      }`}
                    >
                      {t(packer.status.toLowerCase().replace(/\s+/g, "_"))}
                    </span>
                  </td>
                </tr>
              ))}
