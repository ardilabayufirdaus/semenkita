...
import { CementType, PackType, BagType, DowntimeType, PackingPlant, BagBrokenType, User as UserType } from '../types';
...
const packTypes: PackType[] = [
  ...
  { id: '9', code: '50KG-BIGBAG', name: '50 Kg Big Bag (Sling Bag)', description: '' },
];
...
const mockHistoryData: any = {
  CEMENT_STOCK: [
    { date: '2023-10-25', plantId: '1', silo: 'SILO-01', type: 'OPC', stock: 450.5, taker: 'John Doe' },
    { date: '2023-10-25', plantId: '1', silo: 'SILO-02', type: 'PPC', stock: 320.0, taker: 'Jane Smith' },
    { date: '2023-10-24', plantId: '1', silo: 'SILO-01', type: 'OPC', stock: 500.2, taker: 'Mike Ross' },
    { date: '2023-10-25', plantId: '2', silo: 'SILO-A', type: 'PCC', stock: 600.0, taker: 'Admin' },
  ],
  CEMENT_RELEASE: [
    { date: '2023-10-25', plantId: '1', ro: 'RO-23-1001', type: 'PCC', pack: '50KG-HDPE', qty: 500, unit: 'BAGS' },
    { date: '2023-10-25', plantId: '1', ro: 'RO-23-1002', type: 'PPC', pack: '1T-JUMBO', qty: 25.5, unit: 'TON' },
    { date: '2023-10-24', plantId: '1', ro: 'RO-23-0998', type: 'OPC', pack: '50KG-PP', qty: 1000, unit: 'BAGS' },
    { date: '2023-10-25', plantId: '2', ro: 'RO-23-1005', type: 'OPC', pack: '50KG-HDPE', qty: 2000, unit: 'BAGS' },
  ],
  WORKING_HOUR: [
    { date: '2023-10-25', plantId: '1', unit: 'Packer 01', normal: 8.0, overtime: 2.0 },
    { date: '2023-10-25', plantId: '1', unit: 'Packer 02', normal: 8.0, overtime: 0.0 },
    { date: '2023-10-24', plantId: '1', unit: 'Packer 01', normal: 7.5, overtime: 1.5 },
    { date: '2023-10-25', plantId: '2', unit: 'Packer 01', normal: 8.0, overtime: 1.0 },
  ],
  RUNNING_HOUR: [
    { date: '2023-10-25', plantId: '1', unit: 'Packer 01', effective: 6.5, rest: 1.5 },
    { date: '2023-10-25', plantId: '1', unit: 'Packer 02', effective: 7.0, rest: 1.0 },
    { date: '2023-10-25', plantId: '2', unit: 'Packer 01', effective: 7.5, rest: 0.5 },
  ],
  DOWNTIME: [
    { date: '2023-10-25', plantId: '1', category: 'Internal Trouble', type: 'Mekanikal', duration: 45, time: '10:30', remarks: 'Belt realigned' },
    { date: '2023-10-24', plantId: '1', category: 'External Trouble', type: 'Truck N/A', duration: 15, time: '14:15', remarks: 'Waiting for trucks' },
    { date: '2023-10-25', plantId: '2', category: 'Other External Trouble', type: 'PLN', duration: 30, time: '09:00', remarks: 'Power outage' },
  ],
  BAG_STOCK: [
    { date: '2023-10-25', plantId: '1', material: '121-400-0800', in: 5000, out: 4200, damaged: 12, reject: 5 },
    { date: '2023-10-24', plantId: '1', material: '121-400-6070', in: 0, out: 1500, damaged: 2, reject: 0 },
    { date: '2023-10-25', plantId: '2', material: '121-400-0800', in: 2000, out: 1000, damaged: 5, reject: 2 },
  ],
  BAG_BROKEN: [
    { date: '2023-10-25', plantId: '1', material: '121-400-0800', area: 'Mulut', qty: 12 },
    { date: '2023-10-25', plantId: '1', material: '121-400-0620', area: 'Lem', qty: 3 },
    { date: '2023-10-25', plantId: '2', material: '121-400-0800', area: 'Body Kantong', qty: 5 },
  ]
};
...
const EntryData: React.FC<EntryDataProps> = ({ currentUser }) => {
  ...
  const menuItems = [
    { id: 'CEMENT_STOCK', label: 'Cement Stock', icon: Layers, desc: 'Silo levels by Cement Type' },
    { id: 'CEMENT_RELEASE', label: 'Cement Release', icon: Package, desc: 'Outbound by Pack Type' },
    { id: 'WORKING_HOUR', label: 'Working Hours', icon: Clock, desc: 'Normal & Overtime logs' },
    { id: 'RUNNING_HOUR', label: 'Running Hours', icon: Activity, desc: 'Effective vs Rest time' },
    { id: 'DOWNTIME', label: 'Downtime Entry', icon: Timer, desc: 'Stoppage categorization' },
    { id: 'BAG_STOCK', label: 'Bag Stock Control', icon: ShoppingBag, desc: 'In/Out/Reject flows' },
    { id: 'BAG_BROKEN', label: 'Bag Broken Detail', icon: AlertTriangle, desc: 'Waste & damage report' },
  ];
  ...
  const renderHistoryTable = () => {
    ...
    if (data.length === 0) return <div className="p-8 text-center text-slate-500 italic bg-slate-50/50">No records found for the selected period and plant.</div>;
    return (
           ...
           <tbody>
             {data.map((item: any, i: number) => (
               <tr key={i} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                 ...
                 <td className="p-3">
                   {activeForm === 'CEMENT_STOCK' && `${item.silo} - ${item.type}`}
                   {activeForm === 'CEMENT_RELEASE' && `${item.ro} - ${item.pack}`}
                   {activeForm === 'WORKING_HOUR' && `${item.unit}`}
                   {activeForm === 'RUNNING_HOUR' && `${item.unit}`}
                   {activeForm === 'DOWNTIME' && `${item.time} - ${item.category} (${item.type})`}
                   {activeForm === 'BAG_STOCK' && `${item.material}`}
                   {activeForm === 'BAG_BROKEN' && `${item.material} - ${item.area}`}
                 </td>
                 <td className="p-3 text-right font-mono font-bold">
                   {activeForm === 'CEMENT_STOCK' && `${item.stock.toFixed(2)} T`}
                   {activeForm === 'CEMENT_RELEASE' && `${item.qty} ${item.unit}`}
                   {activeForm === 'WORKING_HOUR' && `${item.normal + item.overtime} H`}
                   {activeForm === 'RUNNING_HOUR' && `${item.effective} H`}
                   {activeForm === 'DOWNTIME' && `${item.duration}m`}
                   {activeForm === 'BAG_STOCK' && `In:${item.in}/Out:${item.out}`}
  ...
  const renderFormContent = () => {
    ...
    const inputStyle = "w-full px-3 py-2.5 bg-white border border-slate-300 rounded focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 outline-none text-sm text-slate-900 font-bold shadow-sm transition-all";
    const labelStyle = "block text-xs font-bold text-slate-500 uppercase mb-1";

    switch (activeForm) {
      case 'CEMENT_STOCK':
        const currentPlantSiloCount = mockPlants.find(p => p.id === selectedPlant)?.siloCount || 0;
        const siloOptions = Array.from({ length: currentPlantSiloCount }, (_, i) => `SILO-${String(i + 1).padStart(2, '0')}`);
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               <div>
                  <label className={labelStyle}>Cement Type</label>
                  <select className={inputStyle}>
                    {cementTypes.map(c => <option key={c.id} value={c.id}>{c.code} - {c.name}</option>)}
                  </select>
               </div>
               <div>
                  <label className={labelStyle}>Stock Level (Ton)</label>
                  <input type="number" step="0.01" className={`${inputStyle} font-mono`} placeholder="0.00" />
               </div>
               <div>
                  <label className={labelStyle}>Silo Unit</label>
                  <select className={inputStyle}>
                    {siloOptions.map(s => <option key={s} value={s}>{s}</option>)}
                  </select>
               </div>
               <div>
                  <label className={labelStyle}>Stock Taker (PIC)</label>
                  <input type="text" className={inputStyle} placeholder="Operator Name" />
               </div>
            </div>
          </div>
        );
      case 'CEMENT_RELEASE':
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               <div>
                  <label className={labelStyle}>RO Number / DO</label>
                  <input type="text" className={`${inputStyle} font-mono uppercase`} placeholder="RO-23-XXXX" />
               </div>
               <div>
                  <label className={labelStyle}>Pack Type</label>
                  <select className={inputStyle}>
                    {packTypes.map(p => <option key={p.id} value={p.id}>{p.code} - {p.name}</option>)}
                  </select>
               </div>
               <div>
                  <label className={labelStyle}>Quantity</label>
                  <div className="flex">
                    <input type="number" className="flex-1 px-3 py-2.5 bg-white border border-slate-300 rounded-l focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 outline-none text-sm text-slate-900 font-bold shadow-sm font-mono" placeholder="0" />
                    <select 
                      value={releaseUnit} 
               ...
               <div>
                  <label className={labelStyle}>Cement Type</label>
                  <select className={inputStyle}>
                    {cementTypes.map(c => <option key={c.id} value={c.id}>{c.code}</option>)}
                  </select>
               </div>
            </div>
          </div>
      ...
      case 'WORKING_HOUR':
        const packerCount = mockPlants.find(p => p.id === selectedPlant)?.packerCount || 0;
        const packerOptions = Array.from({ length: packerCount }, (_, i) => `Packer ${String(i + 1).padStart(2, '0')}`);
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               <div>
                  <label className={labelStyle}>Packer Unit</label>
                  <select className={inputStyle}>
               ...
               <div className="grid grid-cols-2 gap-3">
                 <div>
                    <label className={labelStyle}>Normal (Hrs)</label>
                    <input type="number" className={`${inputStyle} font-mono`} placeholder="8.0" />
                 </div>
                 <div>
                    <label className={labelStyle}>Overtime (Hrs)</label>
                    <input type="number" className={`${inputStyle} font-mono`} placeholder="0.0" />
                 </div>
               </div>
            </div>
          </div>
      ...
      case 'RUNNING_HOUR':
        const packerRunCount = mockPlants.find(p => p.id === selectedPlant)?.packerCount || 0;
        const packerRunOptions = Array.from({ length: packerRunCount }, (_, i) => `Packer ${String(i + 1).padStart(2, '0')}`);
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               <div>
                  <label className={labelStyle}>Packer Unit</label>
                  <select className={inputStyle}>
                    {packerRunOptions.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
               </div>
               <div className="grid grid-cols-2 gap-3">
                 <div>
                    <label className={labelStyle}>Effective (Hrs)</label>
                    <input type="number" className={`${inputStyle} font-mono`} placeholder="0.0" />
                 </div>
                 <div>
                    <label className={labelStyle}>Rest/Idle (Hrs)</label>
                    <input type="number" className={`${inputStyle} font-mono`} placeholder="0.0" />
                 </div>
               </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               <div className="col-span-1">
                  <label className={labelStyle}>Downtime Type</label>
                  <select className={inputStyle}>
               ...
               <div>
                  ...
                  <input type="number" className={`${inputStyle} font-mono`} placeholder="0" />
               </div>
               <div className="col-span-2">
                  <label className={labelStyle}>Remarks</label>
                  <input type="text" className={inputStyle} placeholder="Details..." />
               </div>
            </div>
          </div>
        );
      case 'BAG_STOCK':
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               <div>
                  <label className={labelStyle}>Material Number</label>
                  <select className={`${inputStyle} font-mono`}>
                    {bagTypes.map(b => <option key={b.id} value={b.material}>{b.material} - {b.description.substring(0, 20)}...</option>)}
                  </select>
               </div>
               <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-cyan-600 uppercase mb-1">Stock In (+)</label>
                    <input type="number" className="w-full px-3 py-2.5 bg-white border border-cyan-200 rounded focus:ring-1 focus:ring-cyan-600 outline-none text-sm text-slate-900 font-bold shadow-sm font-mono" placeholder="0" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-orange-600 uppercase mb-1">Stock Out (-)</label>
                    <input type="number" className="w-full px-3 py-2.5 bg-white border border-orange-200 rounded focus:ring-1 focus:ring-orange-600 outline-none text-sm text-slate-900 font-bold shadow-sm font-mono" placeholder="0" />
                  </div>
               </div>
               <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-rose-600 uppercase mb-1">Damaged</label>
                    <input type="number" className="w-full px-3 py-2.5 bg-white border border-rose-200 rounded focus:ring-1 focus:ring-rose-600 outline-none text-sm text-slate-900 font-bold shadow-sm font-mono" placeholder="0" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-rose-600 uppercase mb-1">Reject</label>
                    <input type="number" className="w-full px-3 py-2.5 bg-white border border-rose-200 rounded focus:ring-1 focus:ring-rose-600 outline-none text-sm text-slate-900 font-bold shadow-sm font-mono" placeholder="0" />
                  </div>
               </div>
            </div>
        ...
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
               ...
               <div>
                  <label className={labelStyle}>Broken Area</label>
                  <select className={inputStyle}>
               ...
               <div>
                  <label className={labelStyle}>Quantity (Pcs)</label>
                  <input type="number" className={`${inputStyle} font-mono`} placeholder="0" />
               </div>
            </div>
          </div>
    ...
    <div className="grid grid-cols-12 gap-6 h-[calc(100vh-8rem)]">
      ...
      <div className="col-span-3 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
         ...
         <div className="p-4 border-b border-slate-200 bg-slate-50">
            <label className="text-[10px] font-bold text-slate-400 uppercase block mb-2">Select Packing Plant</label>
            <div className="relative">
              ...
              >
                {mockPlants.map(p => <option key={p.id} value={p.id}>{p.plantCode} - {p.plantName}</option>)}
              </select>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 flex items-start space-x-3 transition-colors ${
                    activeForm === item.id ? 'bg-cyan-50 border-r-4 border-cyan-500' : 'hover:bg-slate-50 border-r-4 border-transparent'
                  }`}
                >
                  <Icon className={`w-5 h-5 mt-0.5 ${activeForm === item.id ? 'text-cyan-600' : 'text-slate-400'}`} />
                  <div>
                    <h4 className={`text-sm font-bold ${activeForm === item.id ? 'text-cyan-900' : 'text-slate-700'}`}>{item.label}</h4>
         ...
         <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200">
            <div className="flex items-center space-x-3">
               <div className="p-2 bg-cyan-100 rounded text-cyan-700">
                  {React.createElement(menuItems.find(m => m.id === activeForm)?.icon || Layers, { className: 'w-5 h-5' })}
               </div>
               <div>
                  ...
                  <p className="text-xs text-slate-500 font-mono mt-1">
                    ENTRY_MODE: {viewMode} | PLANT: {mockPlants.find(p => p.id === selectedPlant)?.plantCode}
                  </p>
               </div>
            </div>
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
               <button 
                ...
                className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${
                  viewMode === 'ENTRY' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
               >
                 Data Entry
               </button>
               <button 
                ...
                className={`px-4 py-1.5 rounded-md text-xs font-bold uppercase transition-all ${
                  viewMode === 'HISTORY' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
               >
                 History Log
               </button>
               ...
               <form onSubmit={handleSubmit} className="flex flex-col h-full">
                  {!canEdit && (
                    <div className="absolute inset-x-6 top-6 z-10 bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-center shadow-sm">
                        ...
                        <p className="text-xs font-bold text-amber-800 uppercase">View Only Mode: You do not have permission to edit data.</p>
                    </div>
                  )}

                  <div className={`flex-1 ${!canEdit ? 'mt-14 opacity-60 pointer-events-none select-none grayscale-[0.5]' : ''}`}>
                  ...
                  {canEdit && (
                    <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-end space-x-4">
                        {submitted && <span className="text-sm font-bold text-emerald-600 animate-pulse">Data Saved Successfully!</span>}
                        <button type="button" className="px-6 py-2.5 rounded border border-slate-300 text-slate-600 text-xs font-bold uppercase hover:bg-slate-50 transition-colors">
                            Reset
                        </button>
                        <button type="submit" className="px-6 py-2.5 rounded bg-cyan-600 text-white text-xs font-bold uppercase hover:bg-cyan-700 shadow-lg shadow-cyan-200 transition-colors flex items-center">
