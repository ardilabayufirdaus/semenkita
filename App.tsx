...
const StatCard: React.FC<{
  ...
  <div className="bg-white p-6 rounded shadow-sm border border-slate-200 relative overflow-hidden group hover:shadow-md transition-shadow">
    ...
    <div className="flex justify-between items-start">
      ...
      {/* Glowing Dot */}
      <div
        className={`w-1.5 h-1.5 rounded-full ${accentColor} shadow-[0_0_8px_currentColor] opacity-70`}
      ></div>
    </div>
    ...
    <div className="mt-3">
      ...
      {subValue && (
        <p className="text-xs text-slate-400 font-bold mt-0.5">{subValue}</p>
      )}
    </div>
    ...
    <div className="flex items-center mt-3 border-t border-slate-100 pt-3">
      ...
      <span className="text-xs text-slate-400 ml-2 uppercase font-medium">
        vs Last Month
      </span>
    </div>
  </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <div className="col-span-1 md:col-span-2 lg:col-span-2 mt-2">
        <div className="flex items-center justify-between mb-4 border-b border-slate-200 pb-2">
          <h3 className="text-xs font-bold text-slate-600 uppercase tracking-widest flex items-center">
            ...
            {t("dispatch_volume_overview")}
          </h3>
          <button className="text-[10px] font-bold text-cyan-600 uppercase hover:text-cyan-800 transition-colors">
            {t("view_logistics_report")}
          </button>
        </div>
      ...
      <div className="col-span-1 md:col-span-2 lg:col-span-2 mt-2">
        <div className="flex items-center justify-between mb-4 border-b border-slate-200 pb-2">
          <h3 className="text-xs font-bold text-slate-600 uppercase tracking-widest flex items-center">
            <span className="w-2 h-2 bg-slate-400 rounded-sm mr-2"></span>
            {t("cement_demand_forecast")}
          </h3>
          <button className="text-[10px] font-bold text-cyan-600 uppercase hover:text-cyan-800 transition-colors">
            {t("view_stock_forecast")}
          </button>
        </div>
        <StockForecast />
      </div>
  ...
  const handleLogin = (email: string, password: string) => {
    ...
    if (password === "password123") {
      ...
      localStorage.setItem(
        "semenkita_auth",
        JSON.stringify({ email, userIndex })
      );
    }
  };
          ...
          <div className="flex items-center">
            <div className="flex flex-col">
              ...
              <div className="flex items-center mt-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                <span className="hidden sm:inline">{t("semenkita")}</span>
                <ChevronRight className="w-3 h-3 mx-1" />
                <span className="text-cyan-600">
                  {t("module")}: {currentView}
                </span>
              </div>
            </div>
          </div>
